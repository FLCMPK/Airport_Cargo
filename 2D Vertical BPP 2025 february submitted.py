...
s = model.addVars(Items, Items, vtype=GRB.BINARY, name="s") # 1 if item i is placed on top of item j inside the same bin
...
for i in Items:
    for j in Items:
        if j != i:
            ...
            # # Extended constraint 20% Vertext support - Constraint Extention
            # model.addConstr(x[i] + 0.2 * (L_i[i] * (1 - r[i]) + H_i[i] * r[i]) <= x[j] + L_i[j] * (1 - r[j]) + H_i[j] * r[j] + L_max * e1[i, j])
            # model.addConstr(x[j] <= x[i] + L_i[i] * (1 - r[i]) + H_i[i] * r[i] - 0.2 * (L_i[i] * (1 - r[i]) + H_i[i] * r[i]) + L_max * e2[i, j])
...
# ===============================Solve the problem===============================
model.Params.LogFile = "2DBPP_model_new.log"
...
# Final Print output for pickle files according to the assignment instruction
# print Bin Usage
used_bins = [b for b in Bins if z[b].x >= 0.99]
print(f'Bin Usage: {", ".join(f"Bin {b}" for b in used_bins)}')
...
print('Item number, x coordinate, y coordinate, horizontal length, vertical height')
item_placement = {}
for i in Items:
    length = L_i[i] * (1 - r[i].x) + H_i[i] * r[i].x
    height = L_i[i] * r[i].x + H_i[i] * (1 - r[i].x)
    print(f'Item {i}: x={x[i].x}, y={y[i].x}, length={length}, height={height}')
    item_placement[i] = {'x': x[i].x, 'y': y[i].x, 'length': length, 'height': height}
...
# configure the path to save the pickle file
output_pickle_path = "C:/Users/malyd/OneDrive - Delft University of Technology/1st year/AE4446 Airport and Cargo Operations (202425 Q2)/assignment//Results/results.pickle"
...
for b in B.keys():
    if len(I_b[b]) > 0:
        polygon = patches.Polygon(((cutA_b[b], 0), (B[b]['L'], 0), (B[b]['L'], B[b]['H']), (0, B[b]['H']), (0, cutB_b[b])), closed=True, fill=None, edgecolor='purple', linewidth=3)
        ...
        for i in I_b[b]:
            ax.add_patch(Rectangle((XY_pos[i]['x'],
                ...
                facecolor='orange' if I[i]['perishable'] == 1 else 'green' if I[i]['radioactive'] == 1 else 'gray',
                fill=True,
                edgecolor='blue' if I[i]['fragility'] == 1 else 'red' if I[i]['rotation'] == 0 else 'black',
                lw=2,
                hatch='//' if I[i]['rotation'] == 0 else '..' if I[i]['fragility'] == 1 else None,))
        ...
        ax.set_xticklabels(range(0, B[b]['L'] + 1, 10), rotation=45, ha='right')
        ax.set_yticklabels(range(0, B[b]['H'] + 1, 10), rotation=45, va='top')
        ...
        # Add legend
        legend_patches = [
            patches.Patch(color='gray', label='Generic'),
            patches.Patch(color='orange', label='Perishable'),
            patches.Patch(color='green', label='Radioactive'),
            patches.Patch(facecolor='none', edgecolor='red', hatch='//', label='Non-rotatable'),
            patches.Patch(facecolor='none', edgecolor='blue', hatch='..', label='Fragile'),
            patches.Patch(facecolor='none', edgecolor='blue', hatch='//', label='Non-rot&Frag')
        ]
